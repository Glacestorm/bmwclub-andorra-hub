...
import { ArrowUpRight, CalendarDays, MapPin, FolderArchive, Image as ImageIcon } from "lucide-react";
  ...
  return (
    <PageShell>
      <section className="pt-28 pb-14 section-shell">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto grid lg:grid-cols-[1.2fr_0.8fr] gap-8 items-stretch">
            <div className="glass-dark rounded-[2rem] p-8 md:p-10 text-white relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-6">
                <Link to="/destacats" className="inline-flex items-center text-sm text-white/80 hover:text-white">← {t.back}</Link>
            ...
            <Card className="premium-card rounded-[2rem] p-8 border-0">
              <div className="space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="rounded-2xl bg-secondary/70 p-4">
                    ...
                    <p className="mt-2 font-semibold">{event.destination?.label ?? event.destination?.name ?? event.source.label ?? event.source.name}</p>
                  </div>
                </div>
            ...
            {previewImages.length ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {previewImages.map((image) => (
                  <a key={image.src} href={image.src} target="_blank" rel="noreferrer" className="block rounded-2xl overflow-hidden hover-tilt">
                    <img src={image.src} alt={image.alt} className="h-52 w-full object-cover" loading="lazy" referrerPolicy="no-referrer" />
                  </a>
                ))}
              </div>
            ) : (
      ...
      <section className="pb-20">
        <div className="container mx-auto px-4 max-w-6xl">
          <Card className="premium-card rounded-[2rem] border-0 p-6 md:p-8">
            <div className="flex items-center gap-3 mb-6"><FolderArchive className="h-5 w-5 text-primary" /><h2 className="text-2xl font-bold">{t.archive}</h2></div>
            {relatedArchive.length ? (
              <div className="grid md:grid-cols-2 gap-4">
                {relatedArchive.map((item) => (
                  <a key={item.id} href={item.href} target="_blank" rel="noreferrer" className="rounded-2xl border border-border bg-white/70 p-5 hover:shadow-elegant transition-base block">
